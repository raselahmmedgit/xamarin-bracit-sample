...
namespace hBuddyApp.Services.Storage.LiteDbStorage
{
    ...
    public class StorageService : IStorageService
    {
        ...
        {
            var options = await _optionsProvider.GetAsync().ConfigureAwait(false);

            _filePath = GetFilePath();
            var connectionString = $"Filename={_filePath}";
            ...
            catch (Exception ex)
            {
                ...
                {
                    // If this is an encryption error - do not log it.
        ...
        public async Task DropStorageAsync()
        {
            try
            {
                using (await _dbLock.LockAsync())
            ...
            {
                _logger.LogError(e, "Failed to drop the storage.");
            }
        }
    }
}
