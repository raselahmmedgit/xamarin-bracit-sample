...
namespace hBuddyApp.Services.Http.ExceptionsHandling
{
    public class ErrorResponseHandler : IErrorResponseHandler
    {
        ...
        private static bool TryHandleHttpExceptionByStatusCode(HttpExceptionContext context, out Exception generatedException)
        {
            ...
            {
                HttpStatusCode.UpgradeRequired => new UpdateRequiredException(context.ResponseError, context.ResponseError?.ErrorMessage),
                HttpStatusCode.ServiceUnavailable => new MaintenanceResponseException(context.ResponseError, context.ResponseError?.ErrorMessage),
                HttpStatusCode.Unauthorized => new UnauthorizedResponseException(context.ResponseError, context.ResponseError?.ErrorMessage),
