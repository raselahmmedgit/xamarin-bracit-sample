using hBuddyApp.Features.Chat.Components;
...
namespace hBuddyApp.Features.RapidProFcmPushNotifications
{
    public class RapidProDatabase
    {
        ...
        public Task<List<RapidProMessage>> GetRapidProMessagesByChannelIdAsync(string channelId, string type)
        {
            //Get all messages.
            return _dataBase.Table<RapidProMessage>().Where(x => x.ChannelId == channelId && x.Type == type).OrderBy(o => o.CreatedDateTime).ToListAsync();
        }
        ...
        public Task<int> DeleteAllRapidProMessageAsync()
        {
            ...
            _rapidProContainer.RapidProIsDatabase = false;
