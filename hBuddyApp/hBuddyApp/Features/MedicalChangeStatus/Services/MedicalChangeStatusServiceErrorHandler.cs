...
using hBuddyApp.Services.Http;
using hBuddyApp.Services.Http.ExceptionsHandling;

namespace hBuddyApp.Features.MedicalChangeStatus.Services
{
    public class MedicalChangeStatusServiceErrorHandler : ErrorResponseHandler, IMedicalChangeStatusServiceErrorHandler
    {
            ...
            out Exception generatedException)
        {
            ...
            foreach (var errorItem in errors)
            {
                ...
                {
                    validationErrors.Add(Resources.Localization.ChangeStatusCodeGenerationException_StatusId_ErrorText);
                }
                else if (ResponseErrorCode.StatusChangedOn.IsError(errorItem.ErrorTarget))
                {
                    validationErrors.Add(Resources.Localization.ChangeStatusCodeGenerationException_StatusChangedOn_ErrorText);
                }
            }
