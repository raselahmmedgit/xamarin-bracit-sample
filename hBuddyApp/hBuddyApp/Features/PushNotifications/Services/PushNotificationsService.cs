    ...
    public class PushNotificationsService : IPushNotificationsService
    {
        ...
        public async Task UpdateTokenAsync()
        {
            await Task.Run(async () =>
            {
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(pushNotificationToken?.Token) && _isFirstRun)
                    {
                    ...
                    var currentToken = pushNotificationToken.Token;
                ...
                {
                    _logger.LogErrorExceptCancellation(ex, "Failed to update push notification token for the device");
                }
        ...
        public async Task<bool> IsPermissionGrantedAsync()
        {
            ...
            foreach (var initializer in initializers)
            {
                try
                {
                    result &= await initializer.IsPermissionGrantedAsync();
                    if (!result)
                    {
                        break;
                    }
        ...
        public void OpenAppSettings()
        {
            ...
            {
                initializers[0].OpenAppSettings();
            }
        }
    }
}
